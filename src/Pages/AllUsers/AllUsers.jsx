...
const AllUsers = () => {
    const { data: users = [], refetch } = useQuery(['users'], async () => {
        const res = await fetch("https://musical-camp-server.vercel.app/users")
        return res.json();
    })

    const handleAdmin = (user) => {
        fetch(`https://musical-camp-server.vercel.app/users/admin/${user._id}`, {
            method: 'PATCH'
        })
            .then(res => res.json())
            .then(data => {
    ...
    const handleInstructor = (user) => {
        fetch(`https://musical-camp-server.vercel.app/users/instructor/${user._id}`, {
            method: 'PATCH'
        })
            .then(res => res.json())
            .then(data => {
    ...
    const handleDelete = (user) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`https://musical-camp-server.vercel.app/users/${user._id}`, {
                    method: 'DELETE'
                })
                    .then(res => res.json())
                    .then(data => {
